jQuery(
	function ( $ ) {
		...
		$( '.akismet-user-comment-count' ).each(
			function () {
				var thisId = $( this ).attr( 'commentid' );
		...
		if ( "enable_mshots" in WPAkismet && WPAkismet.enable_mshots ) {
			$( '#the-comment-list' ).on(
				...
				'mouseout',
				'a[id^="author_comment_url"], tr.pingback td.column-author a:first-of-type, td.comment p a',
				function () {
					mshotRemovalTimer = setTimeout(
						function () {
								clearTimeout( mshotRetryTimer );
		...
		function preloadMshotsInViewport() {
			...
			$( '#the-comment-list' ).find( mshotEnabledLinkSelector ).each(
		...
		$( '.checkforspam.enable-on-load' ).on(
			'click',
			function ( e ) {
		 ...
		 */
		$( '.akismet-enter-api-key-box__reveal' ).on(
			'click',
			function ( e ) {
				e.preventDefault();
