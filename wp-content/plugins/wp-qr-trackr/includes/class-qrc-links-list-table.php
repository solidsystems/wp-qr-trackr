...
class QRC_Links_List_Table extends WP_List_Table
{
	...
	protected function referral_filter_dropdown()
	{
		...
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['_wpnonce'] ?? '')), 'bulk-' . $this->_args['plural'])) {
			return;
		}
	...
	private function table_data()
	{
		...
		if (false === $data) {
			...
			if (!empty($where_values)) {
				...
				// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- SQL built safely with placeholders.
				...
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Cached immediately after query, needed for admin display.
				// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- SQL built safely, no user input.
	...
	private function sort_data($a, $b)
	{
		...
		// Verify nonce for admin actions.
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'] ?? '')), 'bulk-' . $this->_args['plural'])) {
			return 0;
		}
