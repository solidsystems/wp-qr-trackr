...
function qrc_activate() {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'qr_trackr_debug_log' ) ) {
		qr_trackr_debug_log( 'QR Trackr activation: Table creation result: ' . wp_json_encode( $result ) );
	}
	...
	if ( $table_exists !== $table_name ) {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'qr_trackr_debug_log' ) ) {
			qr_trackr_debug_log( 'QR Trackr activation: Failed to create table ' . $table_name . '. Last error: ' . $wpdb->last_error );
		}
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'qr_trackr_debug_log' ) ) {
			qr_trackr_debug_log( 'QR Trackr activation: Table ' . $table_name . ' created successfully' );
		}
	}
...
function qrc_create_post_type() {
	...
	// Only register post type if $wp_rewrite is available.
...
function qr_trackr_check_permalink_structure() {
	...
	if ( empty( $permalink_structure ) ) {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'qr_trackr_debug_log' ) ) {
			qr_trackr_debug_log( 'QR Trackr activation: Plain permalinks detected - QR code redirects will not work' );
		}
	} else {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'qr_trackr_debug_log' ) ) {
			qr_trackr_debug_log( 'QR Trackr activation: Pretty permalinks detected - QR code redirects will work correctly' );
		}
	}
}
