...
class Flatted {
	...
	public static function parse( $json, $assoc = false, $depth = 512, $options = 0 ) {
		...
		$set[] = &$value;
		if ( is_array( $value ) ) {
			return self::loop( false, array_keys( $value ), $input, $set, $value );
		}
		if ( is_object( $value ) ) {
			return self::loop( true, self::keys( $value ), $input, $set, $value );
		}
		return $value;
	}
	...
	public static function stringify( $value, $options = 0, $depth = 512 ) {
		...
		$input        = array();
		$output       = array();
		$i            = intval( self::index( $known, $input, $value ) );
		while ( $i < count( $input ) ) {
			$output[ $i ] = self::transform( $known, $input, $input[ $i ] );
			++$i;
		}
	...
	private static function loop( $obj, $keys, &$input, &$set, &$output ) {
		foreach ( $keys as $key ) {
			...
			if ( $value instanceof FlattedString ) {
				self::ref( $obj, $key, $input[ $value->value ], $input, $set, $output );
			}
		}
		return $output;
	}
	...
	private static function relate( &$known, &$input, &$value ) {
		if ( is_string( $value ) || is_array( $value ) || is_object( $value ) ) {
			...
			return self::index( $known, $input, $value );
		}
		return $value;
	}
	...
	private static function ref( $obj, &$key, &$value, &$input, &$set, &$output ) {
		if ( is_array( $value ) && ! in_array( $value, $set, true ) ) {
			$set[] = $value;
			$value = self::loop( false, array_keys( $value ), $input, $set, $value );
		} elseif ( is_object( $value ) && ! in_array( $value, $set, true ) ) {
			$set[] = $value;
			$value = self::loop( true, self::keys( $value ), $input, $set, $value );
		}
		if ( $obj ) {
			$output->$key = &$value;
		} else {
	...
	private static function transform( &$known, &$input, &$value ) {
		...
		if ( is_object( $value ) ) {
			...
			foreach ( $keys as $key ) {
				$object->$key = self::relate( $known, $input, $value->$key );
			}
			return $object;
		}
		return $value;
