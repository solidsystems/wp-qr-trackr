...
final class File extends Renderer {
	 ...
	 */
	private const KEYWORD_TOKENS               = array(
		T_ABSTRACT      => true,
		T_ARRAY         => true,
		T_AS            => true,
		T_BREAK         => true,
	...
	private static array $formattedSourceCache = array();
	...
	private function renderSourceWithLineCoverage( FileNode $node ): string {
		...
		$linesTemplate->setVar( array( 'lines' => $lines ) );
	...
	private function renderSourceWithPathCoverage( FileNode $node ): string {
		...
		foreach ( array_keys( $codeLines ) as $line ) {
			$lineData[ $line + 1 ] = array(
				'includedInPaths'    => array(),
				'includedInHitPaths' => array(),
				'tests'              => array(),
			);
		}
	...
	private function renderBranchStructure( FileNode $node ): string {
		...
		$branchesTemplate->setVar( array( 'branches' => $branches ) );
	...
	private function renderPathStructure( FileNode $node ): string {
		...
		$pathsTemplate->setVar( array( 'paths' => $paths ) );
