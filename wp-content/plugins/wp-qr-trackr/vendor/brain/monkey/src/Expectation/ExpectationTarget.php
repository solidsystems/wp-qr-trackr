...
final class ExpectationTarget {
	...
	public function mockMethodName() {
		...
		switch ( $this->type() ) {
			case self::TYPE_FUNCTION:
				break;
			case self::TYPE_ACTION_ADDED:
				$name = "add_action_{$name}";
				break;
			case self::TYPE_ACTION_DONE:
				$name = "do_action_{$name}";
				break;
			case self::TYPE_ACTION_REMOVED:
				$name = "remove_action_{$name}";
				break;
			case self::TYPE_FILTER_ADDED:
				$name = "add_filter_{$name}";
				break;
			case self::TYPE_FILTER_APPLIED:
				$name = "apply_filters_{$name}";
				break;
			case self::TYPE_FILTER_REMOVED:
	...
	public function equals( ExpectationTarget $target ) {
		return $this->original_name === $target->original_name
