...
class MockConfiguration {
	...
	public function getMethodsToMock() {
		...
		if ( $whiteListedMethods !== array() ) {
			$whitelist = array_map( 'strtolower', $whiteListedMethods );

			return array_filter(
				$methods,
				static function ( $method ) use ( $whitelist ) {
		...
		if ( $blackListedMethods !== array() ) {
			$blacklist = array_map( 'strtolower', $blackListedMethods );

			$methods = array_filter(
				$methods,
				static function ( $method ) use ( $blacklist ) {
	...
	public function getTargetInterfaces() {
		if ( $this->targetInterfaces !== array() ) {
	...
	protected function getAllMethods() {
		...
		$methods = array_filter(
			$methods,
			static function ( $method ) use ( &$names ) {
