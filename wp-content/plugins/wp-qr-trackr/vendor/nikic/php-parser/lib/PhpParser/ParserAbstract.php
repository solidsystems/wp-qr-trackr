...
abstract class ParserAbstract implements Parser {
	...
	public function __construct( Lexer $lexer, ?PhpVersion $phpVersion = null ) {
		...
		$this->dropTokens       = array_fill_keys(
			array( \T_WHITESPACE, \T_OPEN_TAG, \T_COMMENT, \T_DOC_COMMENT, \T_BAD_CHARACTER ),
			true
		);
	}
	...
	public function parse( string $code, ?ErrorHandler $errorHandler = null ): ?array {
		...
		$this->tokenStartStack = array();
		$this->tokenEndStack   = array();
		$this->semStack        = array();
		$this->semValue        = null;
	...
	*/
	...
	private function fixupNamespaceAttributes( Node\Stmt\Namespace_ $stmt ): void {
		...
		// knowledge we have.
		$endAttributes = array( 'endLine', 'endFilePos', 'endTokenPos' );
	...
	protected function fixupArrayDestructuring( Array_ $node ): Expr\List_ {
		...
		return new Expr\List_(
			...
			array( 'kind' => Expr\List_::KIND_ARRAY ) + $node->getAttributes()
		);
	}
	...
	protected function checkClassConst( ClassConst $node, int $modifierPos ): void {
		foreach ( array( Modifiers::STATIC, Modifiers::ABSTRACT, Modifiers::READONLY ) as $modifier ) {
	...
	protected function checkConstantAttributes( Const_ $node ): void {
		if ( $node->attrGroups !== array() && count( $node->consts ) > 1 ) {
			$this->emitError(
				new Error(
