...
function interpretCallable( $callback ) {
	if ( is_object( $callback ) ) {
		return interpretCallable( array( $callback, '__invoke' ) );
	}
	...
	if ( substr( $callback, 0, 4 ) === 'new ' ) {
		return array( ltrim( substr( $callback, 4 ) ), 'new', null );
	}
...
function wildcardMatches( $wildcard, $subject ) {
	return matchWildcard( $wildcard, array( $subject ) ) == array( $subject );
}
...
function getParameterAndArgumentLists( ?\ReflectionMethod $reflection = null ) {
	...
	return array( join( ', ', $parameters ), join( ', ', $arguments ) );
}
