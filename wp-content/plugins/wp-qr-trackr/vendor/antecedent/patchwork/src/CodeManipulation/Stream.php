...
class Stream {
	...
	public static function alternate( callable $internal, $resource, $wrapped, array $args = array(), array $extraArgs = array(), $context = null, $shouldReturnResource = false ) {
		...
		if ( is_object( $resource ) ) {
			...
			$ladder = function () use ( $resource, $wrapped, $args ) {
				switch ( count( $args ) ) {
					...
					default:
						return call_user_func_array( array( $resource, $wrapped ), $args );
				}
			};
			$result = $ladder();
			static::unwrap();
			...
			$result = static::bypass(
				function () use ( $internal, $args ) {
					switch ( count( $args ) ) {
						case 0:
							return $internal();
						case 1:
	...
	public static function fopen( $path, $mode, $options, $context = null ) {
		...
		return static::bypass(
			function () use ( $path, $mode, $options, $context ) {
				if ( $context === null ) {
	...
	public static function fread( $resource, $count ) {
		return static::alternate( 'fread', $resource, 'stream_read', array( $count ) );
	}
	...
	public function url_stat( $path, $flags ) {
		...
		return static::alternate( $internal, null, __FUNCTION__, array( $path, $flags ), array(), $this->context );
	}
	...
	public function dir_opendir( $path, $options ) {
		$this->resource = static::alternate( 'opendir', null, __FUNCTION__, array( $path ), array( $options ), $this->context );
		return $this->resource !== false;
	}
	...
	public function mkdir( $path, $mode, $options ) {
		return static::alternate( 'mkdir', null, __FUNCTION__, array( $path, $mode, $options ), array(), $this->context );
	}

	public function rename( $pathFrom, $pathTo ) {
		return static::alternate( 'rename', null, __FUNCTION__, array( $pathFrom, $pathTo ), array(), $this->context );
	}

	public function rmdir( $path, $options ) {
		return static::alternate( 'rmdir', null, __FUNCTION__, array( $path ), array( $options ), $this->context );
	}
	...
	public function stream_lock( $operation ) {
		...
		return static::alternate( 'flock', $this->resource, __FUNCTION__, array( $operation ) );
	}
	...
	public function stream_set_option( $option, $arg1, $arg2 ) {
		...
		return static::alternate( $internal, $this->resource, __FUNCTION__, array( $option, $arg1, $arg2 ) );
	}
	...
	public static function fwrite( $resource, $data ) {
		return static::alternate( 'fwrite', $resource, 'stream_write', array( $data ) );
	}

	public function unlink( $path ) {
		return static::alternate( 'unlink', $this->resource, __FUNCTION__, array( $path ), array(), $this->context );
	}
	...
	public function stream_metadata( $path, $option, $value ) {
		...
		return static::alternate( $internal, null, __FUNCTION__, array( $path, $option, $value ) );
	}

	public function stream_truncate( $newSize ) {
		return static::alternate( 'ftruncate', $this->resource, __FUNCTION__, array( $newSize ) );
	}
}
