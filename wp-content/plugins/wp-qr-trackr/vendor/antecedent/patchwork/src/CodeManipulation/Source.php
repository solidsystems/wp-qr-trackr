...
class Source {
	...
	function initialize() {
		...
		$this->splices       = array();
		$this->spliceLengths = array();
		$this->cache         = array();
	}

	function indexTokensByType() {
		$this->tokensByType = array();
	...
	function collectLevelInfo() {
		$level                      = 0;
		$this->levels               = array();
		$this->tokensByLevel        = array();
		$this->levelBeginnings      = array();
		$this->levelEndings         = array();
		$this->tokensByLevelAndType = array();
		foreach ( $this->tokens as $offset => $token ) {
			$type = $token[ self::TYPE_OFFSET ];
			switch ( $type ) {
				case '(':
			...
			Utils\appendUnder( $this->tokensByLevelAndType, array( $level, $type ), $offset );
		}
	...
	function within( $types, $low, $high ) {
		$result = array();
		foreach ( (array) $types as $type ) {
			$candidates = isset( $this->tokensByType[ $type ] ) ? $this->tokensByType[ $type ] : array();
	...
	function siblings( $types, $offset ) {
		...
		$begin = Utils\lastNotGreaterThan( Utils\access( $this->levelBeginnings, $level, array() ), $offset );
		$end   = Utils\firstGreaterThan( Utils\access( $this->levelEndings, $level, array() ), $offset );
		if ( $types === self::ANY ) {
			return Utils\allWithinRange( $this->tokensByLevel[ $level ], $begin, $end );
		} else {
			$result = array();
			foreach ( (array) $types as $type ) {
				$candidates = Utils\access( $this->tokensByLevelAndType, array( $level, $type ), array() );
	...
	function next( $types, $offset ) {
		if ( ! is_array( $types ) ) {
			$candidates = Utils\access( $this->tokensByType, $types, array() );
	...
	function all( $types ) {
		if ( ! is_array( $types ) ) {
			return Utils\access( $this->tokensByType, $types, array() );
		}
		$result = array();
		foreach ( $types as $type ) {
	...
	static function junk() {
		return array( T_WHITESPACE, T_COMMENT, T_DOC_COMMENT );
	}
	...
	function cache( array $args, \Closure $function ) {
		...
		$result   = &$this->cache;
		foreach ( array_merge( array( $location ), $args ) as $step ) {
			if ( ! is_scalar( $step ) ) {
