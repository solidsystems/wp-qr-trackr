...
foreach ( array( 'NAME_FULLY_QUALIFIED', 'NAME_QUALIFIED', 'NAME_RELATIVE', 'ELLIPSIS', 'ATTRIBUTE', 'READONLY' ) as $constant ) {
...
function injectCodeAfterClassDefinitions( $code ) {
	return function ( Source $s ) use ( $code ) {
		foreach ( $s->all( T_CLASS ) as $match ) {
			if ( $s->is( array( LEFT_ROUND, LEFT_CURLY, T_EXTENDS, T_IMPLEMENTS ), $s->skip( Source::junk(), $match ) ) ) {
