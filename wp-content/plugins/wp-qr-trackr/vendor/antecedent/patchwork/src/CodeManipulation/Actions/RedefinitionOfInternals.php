...
function spliceNamedCallsWithin( Source $s, $begin, $end, array $names, array $aliases ) {
	foreach ( $s->within( array( T_STRING, Generic\NAME_FULLY_QUALIFIED, Generic\NAME_QUALIFIED, Generic\NAME_RELATIVE ), $begin, $end ) as $string ) {
		...
		if ( isset( $names[ $original ] ) || isset( $aliases[ $original ] ) ) {
			...
			if ( $s->is( array( T_FUNCTION, T_OBJECT_OPERATOR, T_DOUBLE_COLON, T_STRING, T_NEW, Generic\NAME_FULLY_QUALIFIED, Generic\NAME_QUALIFIED, Generic\NAME_RELATIVE ), $previous ) ) {
				continue;
			}
