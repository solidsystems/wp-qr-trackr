...
function spliceAllInstantiations( Source $s ) {
	...
	foreach ( $s->all( T_NEW ) as $new ) {
		...
		if ( $s->is( array( T_CLASS, Generic\READONLY, Generic\ATTRIBUTE ), $begin ) ) {
			// Anonymous class
			continue;
		}
		$end                        = scanInnerTokens( $s, $begin, $dynamic );
		$afterEnd                   = $s->skip( Source::junk(), $end );
		list($argsOpen, $argsClose) = array( null, null );
		if ( $s->is( Generic\LEFT_ROUND, $afterEnd ) ) {
			list($argsOpen, $argsClose) = array( $afterEnd, $s->match( $afterEnd ) );
		}
...
function publicizeConstructors( Source $s ) {
	...
	foreach ( $s->all( array( T_PRIVATE, T_PROTECTED ) ) as $first ) {
