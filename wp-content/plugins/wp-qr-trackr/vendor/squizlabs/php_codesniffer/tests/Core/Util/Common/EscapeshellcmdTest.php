...
final class EscapeshellcmdTest extends TestCase {
	...
	public static function dataEscapeshellcmd() {
		return array(
			'Command is empty string'  => array(
				'command'  => '',
				'expected' => '',
			),
			'Command is simple string' => array(
				'command'  => 'csslint',
				'expected' => 'csslint',
			),
			'Command containing characters which PHP escapes' => array(
				'command'     => '&#;`|*?~<>^()[]{}$\,%!',
				'expected'    => '\&\#\;\`\|\*\?\~\<\>\^\(\)\[\]\{\}\$\\\\,%!',
				'expectedWin' => '^&^#^;^`^|^*^?^~^<^>^^^(^)^[^]^{^}^$^\,^%^!',
			),
			// @link https://github.com/squizlabs/PHP_CodeSniffer/pull/3214
			'Command containing spaces, which can cause problems on Windows' => array(
				'command'     => 'C:\Program Files\nodejs\csslint.cmd',
				'expected'    => 'C:\\\\Program Files\\\\nodejs\\\\csslint.cmd',
				'expectedWin' => 'C:^\Program^ Files^\nodejs^\csslint.cmd',
			),
			// @link https://github.com/php/doc-en/pull/511
			'Command containing spaces with additional arguments' => array(
