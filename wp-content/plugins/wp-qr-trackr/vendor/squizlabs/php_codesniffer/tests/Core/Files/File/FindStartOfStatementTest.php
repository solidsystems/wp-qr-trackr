...
final class FindStartOfStatementTest extends AbstractMethodUnitTest {
	...
	public static function dataFindStartInsideSwitchCaseDefaultStatements() {
		return array(
			'Case keyword should be start of case statement - case itself'                            => array(
				'testMarker'     => '/* testCaseStatement */',
				'targets'        => T_CASE,
				'expectedTarget' => T_CASE,
			),
			'Case keyword should be start of case statement - number (what\'s being compared)'        => array(
				'testMarker'     => '/* testCaseStatement */',
				'targets'        => T_LNUMBER,
				'expectedTarget' => T_CASE,
			),
			'Variable should be start of arbitrary assignment statement - variable itself'            => array(
				'testMarker'     => '/* testInsideCaseStatement */',
				'targets'        => T_VARIABLE,
				'expectedTarget' => T_VARIABLE,
			),
			'Variable should be start of arbitrary assignment statement - equal sign'                 => array(
				'testMarker'     => '/* testInsideCaseStatement */',
				'targets'        => T_EQUAL,
				'expectedTarget' => T_VARIABLE,
			),
			'Variable should be start of arbitrary assignment statement - function call'              => array(
				'testMarker'     => '/* testInsideCaseStatement */',
				'targets'        => T_STRING,
				'expectedTarget' => T_VARIABLE,
			),
			'Break should be start for contents of the break statement - contents'                    => array(
				'testMarker'     => '/* testInsideCaseBreakStatement */',
				'targets'        => T_LNUMBER,
				'expectedTarget' => T_BREAK,
			),
			'Continue should be start for contents of the continue statement - contents'              => array(
				'testMarker'     => '/* testInsideCaseContinueStatement */',
				'targets'        => T_LNUMBER,
				'expectedTarget' => T_CONTINUE,
			),
			'Return should be start for contents of the return statement - contents'                  => array(
				'testMarker'     => '/* testInsideCaseReturnStatement */',
				'targets'        => T_FALSE,
				'expectedTarget' => T_RETURN,
			),
			'Exit should be start for contents of the exit statement - close parenthesis'             => array(
			...
			'Throw should be start for contents of the throw statement - new keyword'                 => array(
				'testMarker'     => '/* testInsideCaseThrowStatement */',
				'targets'        => T_NEW,
				'expectedTarget' => T_THROW,
			),
			'Throw should be start for contents of the throw statement - exception name'              => array(
				'testMarker'     => '/* testInsideCaseThrowStatement */',
				'targets'        => T_STRING,
				'expectedTarget' => T_THROW,
			),
			'Throw should be start for contents of the throw statement - close parenthesis'           => array(
				'testMarker'     => '/* testInsideCaseThrowStatement */',
				'targets'        => T_CLOSE_PARENTHESIS,
				'expectedTarget' => T_THROW,
			),
			'Goto should be start for contents of the goto statement - goto label'                    => array(
				'testMarker'     => '/* testInsideCaseGotoStatement */',
				'targets'        => T_STRING,
				'expectedTarget' => T_GOTO,
			),
			'Goto should be start for contents of the goto statement - semicolon'                     => array(
				'testMarker'     => '/* testInsideCaseGotoStatement */',
				'targets'        => T_SEMICOLON,
				'expectedTarget' => T_GOTO,
			),
			'Default keyword should be start of default statement - default itself'                   => array(
				'testMarker'     => '/* testDefaultStatement */',
				'targets'        => T_DEFAULT,
				'expectedTarget' => T_DEFAULT,
			),
			'Return should be start for contents of the return statement (inside default) - variable' => array(
	...
	public static function dataFindStartInsideClosedScopeNestedWithinMatch() {
		return array(
			...
			'Closure function keyword should be start of closure - closure keyword' => array(
				'testMarker'     => '/* test437ClosureDeclaration */',
				'target'         => T_CLOSURE,
				'expectedTarget' => T_CLOSURE,
			),
			'Open curly is a statement/expression opener - open curly' => array(
				'testMarker'     => '/* test437ClosureDeclaration */',
				'target'         => T_OPEN_CURLY_BRACKET,
				'expectedTarget' => T_OPEN_CURLY_BRACKET,
			),

			'Echo should be start for expression - echo keyword' => array(
				'testMarker'     => '/* test437EchoNestedWithinClosureWithinMatch */',
				'target'         => T_ECHO,
				'expectedTarget' => T_ECHO,
			),
			'Echo should be start for expression - variable' => array(
				'testMarker'     => '/* test437EchoNestedWithinClosureWithinMatch */',
				'target'         => T_VARIABLE,
				'expectedTarget' => T_ECHO,
			),
			'Echo should be start for expression - comma' => array(
				'testMarker'     => '/* test437EchoNestedWithinClosureWithinMatch */',
				'target'         => T_COMMA,
				'expectedTarget' => T_ECHO,
			),
			...
			'First token after comma in echo expression should be start for expression - text string' => array(
				'testMarker'     => '/* test437EchoNestedWithinClosureWithinMatch */',
				'target'         => T_CONSTANT_ENCAPSED_STRING,
				'expectedTarget' => T_CONSTANT_ENCAPSED_STRING,
			),
			'First token after comma in echo expression - PHP_EOL constant' => array(
				'testMarker'     => '/* test437EchoNestedWithinClosureWithinMatch */',
				'target'         => T_STRING,
				'expectedTarget' => T_STRING,
			),
			'First token after comma in echo expression - semicolon' => array(
				'testMarker'     => '/* test437EchoNestedWithinClosureWithinMatch */',
				'target'         => T_SEMICOLON,
				'expectedTarget' => T_STRING,
			),
	...
	public static function dataFindStartInsideParenthesesNestedWithinMatch() {
		return array(
			'Array item itself should be start for first array item'                       => array(
				'testMarker'     => '/* test437NestedLongArrayWithinMatch */',
				'target'         => T_LNUMBER,
				'expectedTarget' => T_LNUMBER,
			),
			'Array item itself should be start for second array item'                      => array(
				'testMarker'     => '/* test437NestedLongArrayWithinMatch */',
				'target'         => T_DNUMBER,
				'expectedTarget' => T_DNUMBER,
			),
			'Array item itself should be start for third array item'                       => array(
				'testMarker'     => '/* test437NestedLongArrayWithinMatch */',
				'target'         => T_VARIABLE,
				'expectedTarget' => T_VARIABLE,
			),

			'Parameter itself should be start for first param passed to function call'     => array(
				'testMarker'     => '/* test437NestedFunctionCallWithinMatch */',
				'target'         => T_LNUMBER,
				'expectedTarget' => T_LNUMBER,
			),
			'Parameter itself should be start for second param passed to function call'    => array(
				'testMarker'     => '/* test437NestedFunctionCallWithinMatch */',
				'target'         => T_VARIABLE,
				'expectedTarget' => T_VARIABLE,
			),
			'Parameter itself should be start for third param passed to function call'     => array(
				'testMarker'     => '/* test437NestedFunctionCallWithinMatch */',
				'target'         => T_DNUMBER,
				'expectedTarget' => T_DNUMBER,
			),

			'Parameter itself should be start for first param declared in arrow function'  => array(
				'testMarker'     => '/* test437NestedArrowFunctionWithinMatch */',
				'target'         => T_VARIABLE,
				'expectedTarget' => T_VARIABLE,
			),
			'Parameter itself should be start for second param declared in arrow function' => array(
