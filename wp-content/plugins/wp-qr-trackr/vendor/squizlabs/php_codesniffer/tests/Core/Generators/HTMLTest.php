...
final class HTMLTest extends TestCase {
	...
	public function testDocSpecifics( $sniffs, $pathToExpected ) {
		...
		$config   = new ConfigDouble( array( "--standard=$standard", "--sniffs=$sniffs" ) );
		$ruleset  = new Ruleset( $config );
		...
		$restrictions = array( $sniffName => true );
	...
	public static function dataDocSpecifics() {
		return array(
			'Documentation title: case'                    => array(
			...
			'Documentation title: length'                  => array(
			...
			'Documentation title: fallback to file name'   => array(
			...
			'Standard Element: blank line handling'        => array(
			...
			'Code Title: line wrapping'                    => array(
			...
			'Code Title: whitespace handling'              => array(
			...
			'Code Comparison: blank line handling'         => array(
			...
			'Code Comparison: different block lengths'     => array(
			...
			'Code Comparison: encoding of special characters' => array(
			...
			'Code Comparison: line length handling'        => array(
			...
			'Unsupported: <code> element at the wrong level' => array(
				'sniffs'         => 'StandardWithDocs.Unsupported.ElementAtWrongLevel',
				'pathToExpected' => __DIR__ . '/Expectations/ExpectedOutputEmpty.txt',
			),
			'Unsupported: one correct elm, one at wrong level' => array(
			...
			'Unsupported: superfluous code element'        => array(
			...
			'Unsupported: unknown element'                 => array(
				'sniffs'         => 'StandardWithDocs.Unsupported.UnknownElement',
				'pathToExpected' => __DIR__ . '/Expectations/ExpectedOutputEmpty.txt',
			),
			'Invalid: code comparison mismatched code elms' => array(
			...
			'Invalid: code comparison only has one code elm' => array(
			...
			'Invalid: code elements have no content'       => array(
			...
			'Invalid: code comparison element has no content' => array(
			...
			'Invalid: code comparison two code elms, one empty' => array(
			...
			'Invalid: code comparison two empty code elms' => array(
			...
			'Invalid: code title attributes are empty'     => array(
			...
			'Invalid: code title attributes missing'       => array(
			...
			'Invalid: documentation title attribute is empty' => array(
			...
			'Invalid: documentation title attribute missing' => array(
			...
			'Invalid: standard element has no content'     => array(
	...
	public function testCallingDeprecatedMethodThrowsDeprecationNotice( $methodName ) {
		...
		} elseif ( method_exists( $this, 'expectExceptionMessageRegExp' ) === true ) {
			// PHPUnit < 8.4.0.
			$this->expectException( $exceptionClass );
	...
	public static function dataCallingDeprecatedMethodThrowsDeprecationNotice() {
		return array(
			...
			'printToc()'    => array( 'printToc' ),
			'printFooter()' => array( 'printFooter' ),
		);
