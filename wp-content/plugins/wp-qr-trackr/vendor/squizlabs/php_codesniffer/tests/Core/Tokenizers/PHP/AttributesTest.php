...
final class AttributesTest extends AbstractTokenizerTestCase {
	...
	public static function dataAttribute() {
		return array(
			...
			'class attribute with param'       => array(
			...
			'class attribute with named param' => array(
			...
			'function attribute with params'   => array(
			...
			'function attribute with arrow function as param' => array(
			...
			'function attribute; multiple comma separated classes' => array(
			...
			'function attribute; multiple comma separated classes, one per line' => array(
			...
			'function attribute; multiple comma separated classes, one per line, with comments' => array(
			...
			'function attribute; using partially qualified and fully qualified class names' => array(
	...
	public static function dataAttributeOnParameters() {
		return array(
			'parameter attribute; single, inline'    => array(
			...
			'parameter attribute; multiple comma separated, inline' => array(
			...
			'parameter attribute; single, multiline' => array(
	...
	public static function dataAttributeOnTextLookingLikeCloseTag() {
		return array(
			'function attribute; string param with "?>"' => array(
				'testMarker'              => '/* testAttributeContainingTextLookingLikeCloseTag */',
				'expectedTokensAttribute' => array(
					array(
						'T_STRING',
						'DeprecationReason',
					),
			...
			'function attribute; string param with "?>"; multiline' => array(
	...
	public function testNestedAttributes() {
		...
		$this->assertSame( array( $attribute => ( $attribute + $outerAttributeLength ) ), $tokens[ ( $attribute + 8 ) ]['nested_attributes'] );
		...
		$test( array_slice( $tokens, ( $attribute + 1 ), 7 ), $outerAttributeLength, array( $attribute => $attribute + $outerAttributeLength ) );
		$test( array_slice( $tokens, ( $attribute + 8 ), 1 ), $innerAttributeLength, array( $attribute => $attribute + $outerAttributeLength ) );
		...
		$test( array_slice( $tokens, ( $attribute + 13 ), 1 ), $innerAttributeLength, array( $attribute => $attribute + $outerAttributeLength ) );
		$test( array_slice( $tokens, ( $attribute + 14 ), 10 ), $outerAttributeLength, array( $attribute => $attribute + $outerAttributeLength ) );
