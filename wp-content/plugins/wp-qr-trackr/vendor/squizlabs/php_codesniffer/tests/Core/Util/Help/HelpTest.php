...
final class HelpTest extends TestCase {
	...
	public function testQaArgumentNamesAreWithinAcceptableBounds() {
		$help = new Help( new ConfigDouble(), array() );
	...
	public function testConstructorInvalidArgumentException() {
		...
		new Help( new ConfigDouble(), array(), array() );
	...
	public static function dataOptionFiltering() {
		...
		return array(
			...
			'Invalid options have no influence' => array(
				'longOptions'  => array(
					'doesnotexist',
					'invalid',
				),
			...
			'Short options passed as long options works fine' => array(
				'longOptions'  => array(
					's',
					'suffix',
					'a',
			...
			'All except one category'           => array(
				'longOptions'  => array_diff( $allLongOptions, array( 'version', 'vv', 'vvv' ) ),
				'shortOptions' => str_replace( array( 'h', 'v' ), '', $allShortOptions ),
				'expected'     => array(
					'Scan targets'           => 8,
					'Rule Selection Options' => 7,
					'Run Options'            => 8,
	...
	public static function dataOptionFilteringSpacerHandling() {
		return array(
			'No spacer at start of category'         => array(
				'longOptions'  => array( 'generator' ),
				'shortOptions' => 'ie',
			),
			'No spacer at end of category'           => array(
				'longOptions'  => array(
					'encoding',
					'tab-width',
				),
			...
			'No consecutive spacers within category' => array(
				'longOptions'  => array(
					'report',
					'report-file',
					'report-report',
	...
	private function verifyDisplayUsage( $cliArgs, $expectedRegex ) {
		$help = new Help( new ConfigDouble( $cliArgs ), array() );
	...
	public static function dataDisplayUsage() {
		return array(
			'Usage without colors' => array(
				'cliArgs'       => array( '--no-colors' ),
				'expectedRegex' => '`^\s*Usage:\s+phpc(bf|s) \[options\] \<file\|directory\>\s+$`',
			),
			'Usage with colors'    => array(
				'cliArgs'       => array( '--colors' ),
	...
	public function testReportWidthCalculations( $reportWidth, $longOptions, $expectedOutput ) {
		$config = new ConfigDouble( array( "--report-width=$reportWidth", '--no-colors' ) );
	...
	public static function dataReportWidthCalculations() {
		...
		return array(
			'Report width small: 40; forces report width to minimum width of 60'                                                    => array(
				'reportWidth'    => 40,
			...
			'Report width is minimum: 60 (= self::MIN_WIDTH)'                                                                       => array(
			...
			'Report width matches length for one line, not the other: 96; only one should wrap'                                     => array(
				'reportWidth'    => 96,
				'longOptions'    => $longOptions,
				'expectedOutput' => PHP_EOL.'Rule Selection Options:'.PHP_EOL
			...
			'Report width matches longest line: 119; the messages should not wrap and there should be no stray new line at the end' => array(
				'reportWidth'    => 119,
				'longOptions'    => $longOptions,
				'expectedOutput' => PHP_EOL.'Rule Selection Options:'.PHP_EOL
					.'  -e                      Explain a standard by showing the names of all the sniffs it includes.'.PHP_EOL
	...
	public static function dataColorizeVariableInput() {
		return array(
			...
			'String without variable element(s)'     => array(
				'input'    => 'This is text',
				'expected' => 'This is text',
			),
			'String with variable element'           => array(
				'input'    => 'This <is> text',
				'expected' => "This \033[36m<is>\033[32m text",
			),
			'String with multiple variable elements' => array(
				'input'    => '<This> is <text>',
				'expected' => "\033[36m<This>\033[32m is \033[36m<text>\033[32m",
			),
			'String with unclosed variable element'  => array(
				'input'    => 'This <is text',
				'expected' => 'This <is text',
			),
			'String with nested elements'            => array(
				'input'    => '<This <is> text>',
				'expected' => "\033[36m<This <is> text>\033[32m",
			),
			'String with nested elements and surrounding text' => array(
	...
	public function testPrintCategoryOptionsNoColor( $input, $expectedRegex ) {
		$config = new ConfigDouble( array( '--no-colors' ) );
		$help   = new Help( $config, array() );

		$reflProperty = new ReflectionProperty( $help, 'activeOptions' );
		$reflProperty->setAccessible( true );
		$reflProperty->setValue( $help, array( 'cat' => $input ) );
		$reflProperty->setAccessible( false );
	...
	public function testPrintCategoryOptionsColor( $input, $expectedRegex ) {
		$config = new ConfigDouble( array( '--colors' ) );
		$help   = new Help( $config, array() );
	...
	public static function dataPrintCategoryOptions() {
		...
		return array(
			'Input: arg, spacer, arg; new lines in description get preserved' => array(
				'input'         => array(
					'short-option'                       => array(
						'argument'    => '-a',
					...
					'long-option-multi-line-description' => array(
			...
			'Input: text, arg, text; multi-line text gets wrapped'            => array(
				'input'         => array(
					...
					'multi-line-text-gets-wrapped' => array(
