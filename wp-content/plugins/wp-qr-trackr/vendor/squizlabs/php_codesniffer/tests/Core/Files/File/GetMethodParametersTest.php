...
final class GetMethodParametersTest extends AbstractMethodUnitTest {
	...
	public static function dataUnexpectedTokenException() {
		return array(
			...
			'function-call-fn-phpcs-3.5.3-3.5.4' => array(
				'commentString'   => '/* testFunctionCallFnPHPCS353-354 */',
				'targetTokenType' => array(
					T_FN,
					T_STRING,
				),
			),
			'fn-live-coding'                     => array(
	...
	public function testInvalidUse( $identifier ) {
		...
		$use = $this->getTargetToken( $identifier, array( T_USE ) );
	...
	public static function dataInvalidUse() {
		return array(
			'ImportUse'      => array( '/* testImportUse */' ),
			'ImportGroupUse' => array( '/* testImportGroupUse */' ),
			'TraitUse'       => array( '/* testTraitUse */' ),
		);
	}//end dataInvalidUse()
	 ...
	 * @return void
	 */
	public function testNoParams( $commentString, $targetTokenType = array( T_FUNCTION, T_CLOSURE, T_FN ) ) {
		$target = $this->getTargetToken( $commentString, $targetTokenType );
	...
	public static function dataNoParams() {
		return array(
			'FunctionNoParams'   => array(
				'commentString' => '/* testFunctionNoParams */',
			),
			'ClosureNoParams'    => array(
				'commentString' => '/* testClosureNoParams */',
			),
			'ClosureUseNoParams' => array(
	...
	public function testClosureUse() {
		...
		$this->getMethodParametersTestHelper( '/* ' . __FUNCTION__ . ' */', $expected, array( T_USE ) );
	}//end testClosureUse()
	 ...
	 * @return void
	 */
	private function getMethodParametersTestHelper( $commentString, $expected, $targetType = array( T_FUNCTION, T_CLOSURE, T_FN ) ) {
