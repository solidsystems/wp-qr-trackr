...
final class RecurseScopeMapDefaultKeywordConditionsTest extends AbstractTokenizerTestCase {
	 ...
	 * @var array<int>
	 */
	protected $conditionStopTokens = array(
		T_BREAK,
		T_CONTINUE,
		T_EXIT,
		T_GOTO,
	...
	public function testSwitchDefault( $testMarker, $openerMarker, $closerMarker, $conditionStopMarker = null, $testContent = 'default', $sharedScopeCloser = false ) {
		...
		$token               = $this->getTargetToken( $testMarker, array( T_MATCH_DEFAULT, T_DEFAULT, T_STRING ), $testContent );
		$tokenArray          = $tokens[ $token ];
		$expectedScopeOpener = $this->getTargetToken( $openerMarker, array( T_COLON, T_OPEN_CURLY_BRACKET, T_SEMICOLON ) );
		$expectedScopeCloser = $this->getTargetToken( $closerMarker, array( T_BREAK, T_CLOSE_CURLY_BRACKET, T_RETURN, T_ENDSWITCH ) );
	...
	public static function dataSwitchDefault() {
		return array(
			...
			'switch_and_default_sharing_scope_closer' => array(
			...
			'switch_and_default_with_nested_if_with_and_without_braces' => array(
	...
	public function testIssue3326() {
		...
		$token      = $this->getTargetToken( '/* testClassConstant */', array( T_SEMICOLON ) );
		$tokenArray = $tokens[ $token ];
