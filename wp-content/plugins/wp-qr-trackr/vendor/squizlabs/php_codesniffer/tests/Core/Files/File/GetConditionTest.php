...
final class GetConditionTest extends AbstractMethodUnitTest {
	 ...
	 */
	protected static $testTargets = array(
	 ...
	 * @var array<string>
	 */
	protected $conditionMarkers = array(
	 ...
	 */
	protected $conditionDefaults = array(
		'T_CLASS'      => false,
		'T_ANON_CLASS' => false,
		'T_INTERFACE'  => false,
		'T_TRAIT'      => false,
	 ...
	 */
	protected static $testTokens = array();
	 ...
	 * @var array<string, int>
	 */
	protected static $markerTokens = array();
	...
	public static function dataGetCondition() {
		return array(
			'testSeriouslyNestedMethod' => array(
	...
	public function testHasConditionMultipleTypes() {
		...
		$result = self::$phpcsFile->hasCondition( $stackPtr, array( T_TRY, T_FINALLY ) );
		$this->assertFalse(
			$result,
			'Failed asserting that "testInException" does not have a "try" nor a "finally" condition'
		);

		$result = self::$phpcsFile->hasCondition( $stackPtr, array( T_TRY, T_CATCH, T_FINALLY ) );
		$this->assertTrue(
			$result,
		...
		$result = self::$phpcsFile->hasCondition( $stackPtr, array( T_ANON_CLASS, T_CLOSURE ) );
		$this->assertFalse(
			$result,
