...
final class DNFTypesParseError2Test extends AbstractTokenizerTestCase {
	...
	public function testBrokenDNFTypeParensShouldAlwaysBeAPairMissingCloseParens( $testMarker ) {
		...
		$unionPtr = $this->getTargetToken( $testMarker, array( T_BITWISE_OR, T_TYPE_UNION ), '|' );
		...
		// is not recognized as a valid type declaration.
		$intersectPtr = $this->getTargetToken( $testMarker, array( T_BITWISE_AND, T_TYPE_INTERSECTION ), '&' );
	...
	public static function dataBrokenDNFTypeParensShouldAlwaysBeAPairMissingCloseParens() {
		return array(
			'OO const type'    => array( '/* testBrokenConstDNFTypeParensMissingClose */' ),
			'OO property type' => array( '/* testBrokenPropertyDNFTypeParensMissingClose */' ),
			'Parameter type'   => array( '/* testBrokenParamDNFTypeParensMissingClose */' ),
			'Return type'      => array( '/* testBrokenReturnDNFTypeParensMissingClose */' ),
		);
	...
	public function testBrokenDNFTypeParensShouldAlwaysBeAPairMissingOpenParens( $testMarker ) {
		...
		// Verify that the unmatched open parenthesis is tokenized as a normal parenthesis.
		$closePtr = $this->getTargetToken( $testMarker, array( T_CLOSE_PARENTHESIS, T_TYPE_CLOSE_PARENTHESIS ), ')' );
	...
	public static function dataBrokenDNFTypeParensShouldAlwaysBeAPairMissingOpenParens() {
		return array(
			'OO const type'    => array( '/* testBrokenConstDNFTypeParensMissingOpen */' ),
			'OO property type' => array( '/* testBrokenPropertyDNFTypeParensMissingOpen */' ),
			'Parameter type'   => array( '/* testBrokenParamDNFTypeParensMissingOpen */' ),
			'Return type'      => array( '/* testBrokenReturnDNFTypeParensMissingOpen */' ),
		);
	...
	public function testBrokenDNFTypeParensShouldAlwaysBeAPairMatchedAndUnmatched( $testMarker ) {
		$tokens   = $this->phpcsFile->getTokens();
		$startPtr = $this->getTargetToken( $testMarker, array( T_OPEN_PARENTHESIS, T_TYPE_OPEN_PARENTHESIS ), '(' );
	...
	public static function dataBrokenDNFTypeParensShouldAlwaysBeAPairMatchedAndUnmatched() {
		return array(
			'OO const type - missing one close parenthesis' => array( '/* testBrokenConstDNFTypeParensMissingOneClose */' ),
			'OO property type - missing one open parenthesis' => array( '/* testBrokenPropertyDNFTypeParensMissingOneOpen */' ),
			'Parameter type - missing one close parenthesis' => array( '/* testBrokenParamDNFTypeParensMissingOneClose */' ),
			'Return type - missing one open parenthesis' => array( '/* testBrokenReturnDNFTypeParensMissingOneOpen */' ),
		);
