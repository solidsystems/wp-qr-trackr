...
final class RecurseScopeMapWithNamespaceOperatorTest extends AbstractTokenizerTestCase {
	 ...
	 * @return void
	 */
	public function testScopeSetting( $testMarker, $tokenTypes, $open = array( T_OPEN_CURLY_BRACKET ), $close = array( T_CLOSE_CURLY_BRACKET ) ) {
	...
	public static function dataScopeSetting() {
		return array(
			'class which extends namespace relative name' => array(
				'testMarker' => '/* testClassExtends */',
				'tokenTypes' => array( T_CLASS ),
			),
			'class which implements namespace relative name' => array(
				'testMarker' => '/* testClassImplements */',
				'tokenTypes' => array( T_ANON_CLASS ),
			),
			'interface which extend namespace relative name' => array(
				'testMarker' => '/* testInterfaceExtends */',
				'tokenTypes' => array( T_INTERFACE ),
			),
			'namespace relative name in function return type' => array(
				'testMarker' => '/* testFunctionReturnType */',
				'tokenTypes' => array( T_FUNCTION ),
			),
			'namespace relative name in closure return type' => array(
				'testMarker' => '/* testClosureReturnType */',
				'tokenTypes' => array( T_CLOSURE ),
			),
			'namespace relative name in arrow function return type' => array(
				'testMarker' => '/* testArrowFunctionReturnType */',
				'tokenTypes' => array( T_FN ),
				'open'       => array( T_FN_ARROW ),
				'close'      => array( T_SEMICOLON ),
			),
		);
