...
class NoSilencedErrorsSniff implements Sniff {
	...
	public function process( File $phpcsFile, $stackPtr ) {
		...
		$contextLength  = 4;
		$endOfStatement = $phpcsFile->findEndOfStatement( $stackPtr, array( T_COMMA, T_COLON ) );
		...
		if ( $this->error === true ) {
			...
			$phpcsFile->addError( $error, $stackPtr, 'Forbidden', array( $found ) );
		} else {
			$error = 'Silencing errors is discouraged; found: %s';
			$phpcsFile->addWarning( $error, $stackPtr, 'Discouraged', array( $found ) );
		}
	}//end process()
}//end class
