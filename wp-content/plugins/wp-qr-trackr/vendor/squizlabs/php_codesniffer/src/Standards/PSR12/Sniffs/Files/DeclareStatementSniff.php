	...
	public function process( File $phpcsFile, $stackPtr ) {
		...
		if ( $tokens[ ( $parenthesis + 1 ) ]['type'] !== 'T_SEMICOLON' ) {
			...
			if ( $tokens[ $token ]['type'] === 'T_OPEN_CURLY_BRACKET' ) {
				...
				$token = $phpcsFile->findNext( array( T_WHITESPACE, T_COMMENT ), ( $token + 1 ), null, true );
				if ( $tokens[ $token ]['type'] === 'T_OPEN_CURLY_BRACKET' ) {
					$curlyBracket = $token;
				}
			}//end if
		...
		if ( $curlyBracket !== false ) {
			...
			$nextToken = $phpcsFile->findNext( array( T_WHITESPACE, T_COMMENT ), ( $closeCurlyBracket + 1 ), null, true );
