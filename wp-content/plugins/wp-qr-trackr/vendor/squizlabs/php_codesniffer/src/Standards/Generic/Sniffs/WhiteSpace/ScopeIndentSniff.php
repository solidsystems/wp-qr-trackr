			...
			*/
			...
			if ( ( $checkToken !== null
				...
				if ( $tokens[ $parenCloser ]['line'] !== $tokens[ $parenOpener ]['line'] ) {
					...
					if ( $condition > 0 && $lastOpenTagCondition === $condition ) {
						...
						$first = $phpcsFile->findFirstOnLine( array( T_WHITESPACE, T_INLINE_HTML ), $lastOpenTag, true );
						if ( $this->debug === true ) {
						...
						if ( $prev !== $first ) {
							...
							$first = $phpcsFile->findFirstOnLine( array( T_WHITESPACE, T_INLINE_HTML ), $prev, true );
							if ( $first !== false ) {
								$prev  = $phpcsFile->findStartOfStatement( $first, T_COMMA );
								$first = $phpcsFile->findFirstOnLine( array( T_WHITESPACE, T_INLINE_HTML ), $prev, true );
							} else {
			...
			if ( ( $checkToken !== null
				...
				if ( isset( $tokens[ $scopeCloser ]['scope_condition'] ) === true ) {
					$first = $phpcsFile->findFirstOnLine( array( T_WHITESPACE, T_INLINE_HTML ), $tokens[ $scopeCloser ]['scope_condition'], true );
					if ( $this->debug === true ) {
						$line = $tokens[ $first ]['line'];
						$type = $tokens[ $first ]['type'];
