...
class PHP extends Tokenizer {
	 ...
	 * @var array
	 */
	protected $tstringContexts = array(
	...
	protected function tokenize( $string ) {
		...
		for ( $stackPtr = 0; $stackPtr < $numTokens; $stackPtr++ ) {
			...
			*/
			...
			if ( $tokenIsArray === true
				&& in_array( $token[0], array( T_PUBLIC, T_PROTECTED, T_PRIVATE ), true ) === true
			...
			*/
			...
			if ( $tokenIsArray === true
				...
				$bracketCloser = $this->findCloser( $tokens, ( $stackPtr + 1 ), array( '[', '#[' ), ']' );
			...
			if ( $tokenIsArray === false && $token[0] === '?' ) {
				...
				for ( $i = ( $stackPtr - 1 ); $i >= 0; $i-- ) {
					...
					if ( $tokenType === T_FUNCTION
						...
						$newToken['type'] = 'T_NULLABLE';
						break;
					} elseif ( in_array( $tokenType, array( T_DOUBLE_ARROW, T_OPEN_TAG, T_OPEN_TAG_WITH_ECHO, '=', '{', ';' ), true ) === true ) {
						if ( PHP_CODESNIFFER_VERBOSITY > 1 ) {
							echo "\t\t* token $stackPtr changed from ? to T_INLINE_THEN" . PHP_EOL;
						}
			...
			*/
			...
			if ( $tokenIsArray === true && strpos( $token[1], $this->eolChar ) !== false ) {
				...
				if ( $tokenIsArray === true && $token[0] === T_STRING ) {
					...
					} elseif ( isset( $this->tstringContexts[ $finalTokens[ $lastNotEmptyToken ]['code'] ] ) === true
						&& $finalTokens[ $lastNotEmptyToken ]['code'] !== T_CONST
					) {
						$preserveTstring = true;
		...
		for ( $i = ( $numTokens - 1 ); $i >= 0; $i-- ) {
			...
			if ( $this->tokens[ $i ]['code'] === T_FUNCTION ) {
				...
				*/
				...
				continue;
			} elseif ( $this->tokens[ $i ]['code'] === T_STATIC ) {
				for ( $x = ( $i - 1 ); $x > 0; $x-- ) {
					if ( isset( Tokens::$emptyTokens[ $this->tokens[ $x ]['code'] ] ) === false ) {
						break;
					}
	...
	private function parsePhpAttribute( array &$tokens, $stackPtr ) {
		...
		array_splice( $subTokens, 0, 1, array( array( T_ATTRIBUTE, '#[' ) ) );
