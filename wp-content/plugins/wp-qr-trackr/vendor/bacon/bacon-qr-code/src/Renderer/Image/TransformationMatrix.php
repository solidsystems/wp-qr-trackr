...
final class TransformationMatrix {
	...
	public static function scale( float $size ): self {
		$matrix         = new self();
		$matrix->values = array( $size, 0, 0, $size, 0, 0 );
		return $matrix;
	}

	public static function translate( float $x, float $y ): self {
		$matrix         = new self();
		$matrix->values = array( 1, 0, 0, 1, $x, $y );
		return $matrix;
	}
	...
	public static function rotate( int $degrees ): self {
		$matrix         = new self();
		$rad            = deg2rad( $degrees );
		$matrix->values = array( cos( $rad ), sin( $rad ), -sin( $rad ), cos( $rad ), 0, 0 );
		return $matrix;
	}
