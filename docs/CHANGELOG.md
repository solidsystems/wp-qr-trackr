...
- QR Code Generation: Migrated to fully local generation using Endroid QR Code library (no third-party services).
...
- Removed all references to Google Charts and external QR services.
...
- Historical note updated: implementation now uses local Endroid generation exclusively.
...
- **Fixed fatal error on plugin activation** - Resolved constant mismatch issues between QRC* and QR_TRACKR* prefixes
...
- **Added backward compatibility constants** - Defined legacy QRC\_ constants for existing module compatibility
...
- **Updated plugin constants** - Changed from QRC* to QR_TRACKR* prefix for consistency
