...
## QR Code URL Handling: Clean URLs with Native Redirects

### Final Approach

We use clean, SEO-friendly URLs handled by WordPress rewrites and `template_redirect`:

```
http://localhost:8081/qr/H4qMunPg/
```

### What Was Wrong Earlier (and Fixes)

- Pretty permalinks were not set and rewrite rules weren’t flushed → Fixed by setting a structure and flushing on activation.
- The `template_redirect` handler was disabled temporarily → Re-enabled and guarded with `is_admin()`/`wp_doing_ajax()` early returns.
- External destinations used `wp_safe_redirect()` → Switched to `wp_redirect( esc_url_raw( $destination_url ), 302 )` per security policy.

### Working Implementation

```php
// Rewrite rules (registered on init)
add_rewrite_rule(
    'qr/([a-zA-Z0-9]+)/?$',
...
add_action( 'template_redirect', function() {
    ...
    // Lookup destination and redirect (external allowed)
    $destination_url = qr_trackr_lookup_destination( $qr_code );
...
- Admin-ajax redirect URLs are NOT used in production/nonprod.
- A temporary developer fallback may be enabled behind an explicit feature flag for troubleshooting only.

### Key Learnings

1. Clean URLs work reliably when permalinks are set and rewrites are flushed on activation.
2. Use `template_redirect` with early returns for admin/ajax contexts.
3. Use `wp_redirect()` for external destinations and `wp_safe_redirect()` for internal/admin redirects.
